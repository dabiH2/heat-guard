...
OSHA documents outdoor-worker heat-stroke deaths at a daily maximum heat index of only **86 °F** (**30 °C** — the API takes Celsius, see below) — inside the "Caution" band [[EV:HARM-04]]. Every occupational heat limit in force — NIOSH's RAL and REL, ACGIH's TLV, ISO 7933 — is defined as time at a condition, and NIOSH removed its peak-temperature ceiling limit in 2016 [[EV:NIOSH-01]] [[EV:ACGIH-01]] [[EV:NIOSH-02]] [[EV:ISO-01]]. A daily peak cannot be compared against any of them; hours above a threshold can. **The earlier claim here — "peak temperature is a poor predictor of harm" — was withdrawn 2026-08-29 because it could not be sourced as an empirical fact (see `docs/practice-and-efficacy-evidence.md` §5); do not reintroduce it.** Ask a duration question, answer it with `analytic_type=snapshot` + `filter_type=1`, and you get a plausible single temperature instead of "five straight hours above threshold" — same shape of output, opposite operational decision, **no error raised**.

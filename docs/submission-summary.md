...
because their only heat signal is one city-wide forecast high applied to every site,
which results in 643 phantom worker-hours of "unsafe exposure" on a single measured day
— a 92% over-count worth roughly $35,000 of unnecessary stop-work.**
...
**Is AI generally required?** No — and that is the design. **The router is
deterministic**: a literal decision table maps question to analysis layer, states why,
and refuses when the data cannot answer. The model narrates; it never picks a layer,
threshold or date. A test runs the same question with and without a model and asserts
every decisive field is identical.

**Simplest version to prove it in 24 hours.** One site, one date, two layers side by
side — the "⚠️ The trap" tab. It still runs.
...
The dangerous window ran 13:00–20:00; nearly every shift misses it. Eight sites tie at
7.0 h — identical on any heat map. The worst site is not the hottest but the one with
...
customer discovery has happened** — the roster is constructed, not observed. The
mechanism is measured; the demand is not. Also found: an unconverted Fahrenheit
threshold returns **0.0 hours where the truth is 17.0** — `Completed`, billed, silent.
Five further silent-and-billed failure modes are in `docs/api-notes.md`.

**AI disclosure:** built with Claude Code (Claude Opus 5) — code, tests, docs, API
probing. The pitch video is my own.
